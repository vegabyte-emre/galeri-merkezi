...
public class TrustCerts63 : ICertificatePolicy {
    public bool CheckValidationResult(ServicePoint sp, X509Certificate cert, WebRequest req, int problem) { return true; }
}
"@
[System.Net.ServicePointManager]::CertificatePolicy = New-Object TrustCerts63
...
# Panel container'i bul
$panel = $null
foreach ($c in $containers) {
    $name = $c.Names[0]
    if ($name -match "panel") {
        $panel = $c
        Write-Output "Panel bulundu: $name - ID: $($c.Id.Substring(0,12))"
    }
}
...
if ($panel) {
    ...
    try {
        Invoke-RestMethod -Uri "$portainerUrl/api/endpoints/$endpointId/docker/containers/$($panel.Id)/stop" -Headers $headers -Method Post
    ...
    try {
        Invoke-RestMethod -Uri "$portainerUrl/api/endpoints/$endpointId/docker/containers/$($panel.Id)?force=true" -Headers $headers -Method Delete
