/**
 * Vehicle Catalog SQL Generator v3
 * 
 * Fixes: Properly reads Alt Model and Trim columns even when some cells are empty
 * 
 * Hierarchy: Class → Brand → Series → Model → Alt Model → Trim (optional)
 * 
 * Logic:
 * - Each row in Excel represents a unique vehicle spec
 * - Alt Model may have multiple Trims with different specs
 * - If Alt Model has multiple named Trims, user must select Trim
 * - If Alt Model has single/no named Trim, specs are auto-filled
 */
...
// Read with defval to include empty cells
...
const normalizeFuel = (str) => {
    ...
    if (s.includes('lpg') || s.includes('& lpg')) return 'Benzin & LPG';
...
const normalizeTransmission = (str) => {
    ...
    if (s.includes('yarı')) return 'Yarı Otomatik';
    return normalize(str) || null;
};
const normalizeBodyType = (str) => {
    const s = normalize(str).toLowerCase();
    if (s.includes('sedan')) return 'Sedan';
    if (s.includes('hp 5') || s.includes('5 kapı')) return 'Hatchback 5 Kapı';
    if (s.includes('hp 3') || s.includes('3 kapı')) return 'Hatchback 3 Kapı';
    if (s.includes('hatchback')) return 'Hatchback';
    if (s.includes('station') || s.includes('sw') || s.includes('wagon')) return 'Station Wagon';
    if (s.includes('suv')) return 'SUV';
    if (s.includes('crossover')) return 'Crossover';
    if (s.includes('coupe') || s.includes('coupé')) return 'Coupe';
    if (s.includes('cabrio') || s.includes('roadster') || s.includes('convertible')) return 'Cabrio';
    if (s.includes('mpv') || s.includes('minivan')) return 'MPV';
...
data.forEach(row => {
    ...
    const motorGucu = normalize(row['Motor Gücü '] || row['Motor Gücü']);
...
// Count trims with names
const trimsWithNames = trims.filter(t => t.name && t.name !== '');
console.log('  Trims with names:', trimsWithNames.length);

// Calculate trim counts for alt models
const altModelTrimCounts = new Map();
trims.forEach(t => {
    if (t.altModelId && t.name) {
        const key = t.altModelId;
        if (!altModelTrimCounts.has(key)) {
            altModelTrimCounts.set(key, new Set());
        }
        altModelTrimCounts.get(key).add(t.name);
    }
});

const multiTrimAltModels = Array.from(altModelTrimCounts.entries()).filter(([id, names]) => names.size > 1);
console.log('  Alt Models with multiple named trims:', multiTrimAltModels.length);

// Show examples
console.log('');
console.log('Examples of Alt Models with multiple trims:');
multiTrimAltModels.slice(0, 5).forEach(([altModelIdVal, names]) => {
    const altModelEntry = Array.from(altModels.values()).find(am => am.id === altModelIdVal);
    if (altModelEntry) {
        console.log(`  - ${altModelEntry.name}: ${Array.from(names).join(', ')}`);
    }
});
...
// Generate SQL
let sql = '-- Auto-generated vehicle catalog from Excel (v3)\n';
sql += '-- Source: ' + files[0] + '\n';
...
-- If alt_model_id has multiple trims with different names, user must select one
...
sql += '-- Note: If an alt_model has multiple trims with different names, user must select trim\n';
...
sql += `-- Trims with names: ${trimsWithNames.length}\n`;
sql += `-- Alt Models with multiple named trims: ${multiTrimAltModels.length}\n`;

// Write to file
const outputPath = path.join(rootDir, 'docker', 'catalog-migration-v3.sql');
